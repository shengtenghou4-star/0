...
def sha256(p:Path)->str:
    ...
    return h.hexdigest()
root=Path('s41_artifacts'); raw=root/'raw'; raw.mkdir(parents=True,exist_ok=True)
assert not any(x in URL for x in FORBIDDEN)
target=raw/FILENAME
urllib.request.urlretrieve(URL,target)
obs=(target.stat().st_size,sha256(target))
if obs!=(EXPECTED_BYTES,EXPECTED_SHA256): raise RuntimeError(obs)
manifest={'schema':'bio-001-s41-source-manifest-v1','target':{'filename':FILENAME,'url':URL,'bytes':obs[0],'sha256':obs[1]},
