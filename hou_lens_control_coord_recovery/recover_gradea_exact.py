...
REQUEST_SHA = "b18615d7f8a1e3be27baad05171edb7db38569d7901fecf80d74899ce5403caf"
RESULT_CERT_SHA = "3074cc6af3830060533d58d0f3b6aefc4c2d1dfaac4f257e2ab0590a8033b7e5"
...
def query_batch(session: requests.Session, ids: list[str], batch_index: int) -> tuple[list[dict], dict]:
    ...
    query = f'SELECT "ID", "RAJ2000", "DEJ2000" FROM {TABLE} WHERE "ID" IN ({quoted})'
...
def main() -> None:
    ...
    if request.get("request_count") != 48 or len(rows) != 48:
        fail("request-count gate failed")
    if request.get("matching_permitted") is not False or request.get("selection_permitted") is not False:
        fail("request scientific-boundary gate failed")
    if request.get("cube_sha_consulted") is not False or request.get("scores_consulted") is not False:
        fail("request leakage gate failed")
    ...
    if any(row["family"] != "gradea_prospective16" for row in rows):
        fail("request-family gate failed")

    PRIVATE_OUT.mkdir(exist_ok=True)
    session = requests.Session()
    session.headers["User-Agent"] = "HOU-LENS-P4.9.8-GradeA-control-coordinate-provenance-recovery/1.0"
    ...
    for offset in range(0, len(ids), BATCH):
        ...
        print(f"batch {offset // BATCH + 1:02d}/06 exact-ID coordinates recovered; total={len(recovered_by_id)}/48", flush=True)
    ...
    result = {
        "schema_version": 1,
        "protocol": "HOU-LENS-P4.9.8-GRADEA-CONTROL-COORDINATE-PROVENANCE-RECOVERY-VIZIER",
        "status": "PASS_EXACT_48_OF_48_SOURCE_ID_COORDINATE_RECOVERY",
        ...
        "claim_boundary": "Exact source-ID lookup of authoritative catalog coordinates for already-frozen P4.9.8 Grade-A controls only.",
    }
    result_path = PRIVATE_OUT / "coordinate_recovery.json"
    ...
    public = {
        ...
        "recovered_count": 48,
        "batch_count": len(batch_receipts),
        "private_result_sha256": sha_file(result_path),
        "matching_performed": False,
        "selection_performed": False,
        "cube_sha_consulted": False,
        "scores_consulted": False,
